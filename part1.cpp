...
/*
CLL121 CODING PROJECT [PART-1]
*/

float p1, p2, m1, m2, t, p_i, a_i, a_f;
...
float func(float p, float a, float A, float B, float c1, float c2)
{
    if (p > p1 - 50)
    {
        return p1;
    ...
    float c3 = A * c2 - B * c1;
    float res = B * g1(p, a) - A * g2(p, a) + c3;
    return res;
}

int main()
{
    in >> p1 >> p2 >> m1 >> m2 >> t >> p_i >> a_i >> a_f;
    ...
    float x_l = p2, x_r = p1, x;
    x = x_l;
    while (abs(x_l - x_r) >= 0.0001 && x <= p1 - 50)
    {
        x = 0.5 * (x_l + x_r);
        if (func(x_l, a_f, A, B, c1, c2) * func(x, a_f, A, B, c1, c2) < 0)
        {
            x_r = x;
        }
        else
    ...
    // Accumulator pressure cannot exceed the value of (P1-50) kPa. If it does, then it is an absurd answer
    if (x > p1 - 50)
    {
        out << "No physically valid soln exists." << endl;
    }
    else
    {
        float p_f = x;
        float Vacc = (float)A / (g1(p_f, a_f) - g1(p_i, a_i));
        out << "Pf = " << x << " kPa" << endl;
